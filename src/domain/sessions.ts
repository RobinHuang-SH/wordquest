...
export const storyLengthLabels: Record<StoryLength,{range:string;minutes:number;name:string}> = {
...
export const createInitialState = (): AppState => ({
  ...
  quizAnswers:{},quizDone:false,storyChoice:'',completed:false,streak:7,wordMix:'15+5',storyLength:'medium',
  dailyMinutes:20,activeDate:'',sessions:{},
})
...
export function getPreviousSession(state:AppState) { return state.sessions[addDays(state.activeDate,-1)] }
...
export function resetForNewDay(state:AppState,today=getDateKey()):AppState {
  ...
  return {...state,activeDate:today,learned:{},currentWord:0,quizAnswers:{},quizDone:false,storyChoice:'',completed:false}
}

export function createDailySession(state:AppState,storyChoice:string,completedAt?:string):DailySession {
  const reviewCount=getReviewCount(state)
  return {
    date:state.activeDate,learned:{...state.learned},learnedCount:Object.keys(state.learned).length,
    newCount:20-reviewCount,reviewCount,quizScore:getQuizScore(state.quizAnswers),storyChoice,
    storyLength:state.storyLength,dailyMinutes:state.dailyMinutes,completedAt:completedAt||new Date().toISOString(),
  }
}
...
export function completeDailySession(state:AppState,storyChoice:string):AppState {
  ...
  return {...state,storyChoice,completed:true,sessions:{...state.sessions,[state.activeDate]:session}}
}

export function createWeeklyReport(state:AppState):WeeklyReport {
  const keys=getWeekDateKeys(state.activeDate),sessions=keys.map(key=>state.sessions[key]).filter((session):session is DailySession=>Boolean(session))
  return {startDate:keys[0],endDate:keys[6],learnedWords:sessions.reduce((sum,session)=>sum+session.learnedCount,0),storyCount:sessions.length,sessions}
}
